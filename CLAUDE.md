...
AI Resume Screener is a web application that uses NLP (Natural Language Processing) to match resumes against job descriptions. It extracts text from PDF resumes, calculates similarity scores using a multi-signal approach (TF-IDF cosine similarity, keyword coverage, and breadth bonus), and provides human-readable explanations with recommendations.

**Tech Stack:**
- Backend: FastAPI (Python) with scikit-learn for NLP
- Frontend: Vanilla JavaScript
- PDF Processing: PyPDF2 and pdfplumber
- Text Processing: NLTK for stopword removal and tokenization
...
3. Save PDF temporarily to uploads/ with UUID filename
4. Extract text from PDF (PyPDF2 fallback to pdfplumber)
5. Clean and preprocess text (lowercase, remove stopwords)
6. Calculate multi-signal match score:
   - TF-IDF cosine similarity (40% weight)
   - Keyword coverage ratio (40% weight)
   - Vocabulary breadth bonus (20% weight)
7. Extract matched keywords using TF-IDF scores
8. Generate human-readable explanation with recommendations
9. Return JSON response
10. Cleanup: delete temporary PDF in finally block
```
...
**nlp_matcher.py**
- Multi-signal scoring: 40% TF-IDF cosine similarity, 40% keyword coverage, 20% breadth bonus
...
- `FileMetadata`: File info (filename, size, upload_timestamp)
- `HealthCheckResponse`: Health endpoint response

Helper function:
- `create_success_response()`: Builds successful analysis response with timestamp
...
### NLTK Data
- nlp_matcher.py auto-downloads required data (punkt tokenizer, stopwords) on first import
...
- Docker setup uses non-root user (appuser) and includes HEALTHCHECK
- Test dependencies are in `requirements-dev.txt`, production deps in `requirements.txt`
